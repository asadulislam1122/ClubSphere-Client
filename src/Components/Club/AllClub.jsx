...
const AllClub = () => {
  ...
  return (
    <div className="min-h-screen p-4 md:p-10 bg-base-100 transition-colors duration-500">
      <title>All-Club-Page</title>
      ...
      <div className="max-w-[1400px] mx-auto mb-16 text-center">
        ...
        <h2 className="text-5xl font-black text-base-content mt-4 mb-4">
          Join a{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-500">
            Club
          </span>
      ...
      {/* Modern Grid with GSAP ref */}
      <div
        ref={gridRef}
        className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8"
      >
        {clubs.map((club) => (
          <div
            key={club._id}
            className="club-card group relative bg-base-200/40 backdrop-blur-md border border-base-content/5 rounded-[2rem] p-3 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)]"
          >
            {/* Image Section */}
            <div className="relative h-48 w-full overflow-hidden rounded-[1.5rem] z-10 shadow-md">
              <img
            ...
            <div className="px-3 pt-5 pb-2 text-left">
              <h3 className="text-xl font-extrabold text-base-content leading-tight min-h-[56px]">
                {club.clubName}
              </h3>

              <p className="text-base-content/60 text-xs mt-2 line-clamp-3 font-medium">
                {club.description}
              </p>

              {/* Info Pills */}
              <div className="flex items-center justify-between mt-6 bg-base-300/50 rounded-2xl p-3 border border-base-content/5">
                <div className="flex flex-col">
                  <span className="text-[10px] text-base-content/40 uppercase font-bold">
                    Location
                  </span>
                  <div className="flex items-center gap-1 text-base-content font-bold text-xs">
                    <FaMapMarkerAlt className="text-blue-500" /> {club.location}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <div className="flex items-center gap-1 text-base-content font-black text-sm">
                    <FaDollarSign className="text-emerald-500 text-xs" />{" "}
                    {club.membershipFee}
                  </div>
                </div>
              ...
              >
                ...
                {/* Background circle animation (হোভার করলে এটি বড় হয়ে পুরো বাটনে ছড়িয়ে যায়) */}
                <div className="absolute top-0 right-0 w-16 h-16 bg-white/10 rounded-full -mr-8 -mt-8 transition-all duration-500 group-hover:scale-[6] ease-in-out" />
              </Link>
              {/* ------------------------------------- */}
            </div>
          </div>
